public class Cesar implements Mode {
    ...
    private static int posChar(char c, char[] tab)  { // Renvoie la position du caractere dans le tableau ou -1 si il n'est pas dans le tableau
        for(int i = 0; i < tab.length; i++)   {
            if(tab[i] == c) return i;
        }   return -1;
    }

    private static int newPos(int pos,int n)  { // Donne la nouvelle position dans l'alphabet en fonction de n
        int pos2 = pos;
    ...
    public static int abs(int a)  { // Valeur absolue de a
        if(a >= 0) return a;
        else return -a;
    }
}
